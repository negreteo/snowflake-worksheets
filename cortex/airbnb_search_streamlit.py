# Install packages: snowflake, snowflake-snowpark-python, streamlit
# NOTE: Enable first the cortex search service and index
# NOTE: Enter a text such "affordable" as for "Query"
...
SCHEMA = "AI_POC"
SERVICE = "AIRBNB_SVC"
BASE_TABLE = "dev.ai_poc.airbnb_listings"
ARRAY_ATTRIBUTES = {"AMENITIES"}

# Uses a DESCRIBE SQL query to get information about the attributes available in the search service and stores them in Stremalit state.
...
# Handles querying the Cortex Search Service via the Python client library.
...
# Determines which values are possible for each filterable attribute to populate the dropdown menus.
@st.cache_data
...
# Formats search results into Markdown elements displayed in the results page.
...
# Processes selected filter attributes from the Streamlit form into the right objects to be used by the Python library for querying Cortex Search.
